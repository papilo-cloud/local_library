...
exports.genre_detail = asyncHandler(async (req, res, next) => {
  const [genre, booksInGenre] = await Promise.all([
    ...
    Book.find({genre: req.params.id}, 'title semmary').exec()
  ])
  if (genre == null){
...
exports.genre_create_post =[ 
  body('name', 'Gnere name must contain at least 3 characters')
    .trim()
    .isLength({min: 2})
    .escape(),
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      const genreExists = await Genre.findOne({name: req.body.name})
        ...
        .exec()
      if (genreExists) {
        // Genre exists, Reidrect to its detail page.
        res.redirect(genreExists.url)
      }
      else{
        await genre.save()
        // New genre saved. Reidrect to genre detail page.
...
exports.genre_delete_post = asyncHandler(async (req, res, next) => {
  ...
  if (allBooksByGenre.length > 0) {
    ...
    await Genre.findByIdAndDelete(req.body.genre)
    res.redirect('/catalog/genres')
  }
});
